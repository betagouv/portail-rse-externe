...
class ExtractTexts():
    ...
    def extract_text_from_pdf(self, pdf_path):
        ...
        for page_num in range(self.num_page_debut, min(len(document), self.num_page_fin), 1):

            # Get the page
            page = document.load_page(page_num)

            # Extract text from the page
            page_text = page.get_text(option="text", flags=fitz.TEXT_PRESERVE_WHITESPACE)
    ...
    def process_footer(self, paragraphs_per_pages):
        ...
        is_ordered = all(deduplicated_numbers[i] <= deduplicated_numbers[i+1] for i in range(len(deduplicated_numbers)-1))

        # Drop the first part of paragraph if it'a a Footer
        if is_ordered:
          for i, paragraphs in enumerate(paragraphs_per_pages):
            if digits[i] > -1:
              paragraphs_without_footer = self.drop_first_carriage_return_split(paragraphs[0])
    ...
    def split_into_segments(self, paragraph):
        ...
        segments = re.split(r'(?<!\w\.\w.)(?<![A-Z][a-z]\.)(?<=\.|\?)(?<!\b\w\w\.)\s', paragraph)
        return segments
    ...
    def identify_segment_type(self, segment):
        ...
        if '\n' in segment:
            lines = segment.split('\n')
            number_lines = sum(1 for line in lines if re.fullmatch(r'\d+', line.strip()))
        ...
        words = segment.split()
        if all(word[0].isupper() for word in words) and not segment.endswith('.') and len(segment) < 50:
            return "title"
    ...
    def clean_text(self, segment):
        ...
        clean_segment = ''.join(c if c.isprintable() or c == "\n" else " " for c in segment)
        ...
        clean_segment = re.sub(r'[^a-zA-Z0-9À-ÿçÇñÑàâäéèêëîïôöùûüÿÿ\'’" ,.;:!?()\[\]\{\}\-\+/%&$€*=^|~π√∫∆∑∏<>≤≥\n\rﬁ]+', '', clean_segment)
    ...
    def exclude_upper_or_space_segments(self, text):
        ...
        filtered_parts = [part for part in parts if not (re.match(pattern, part) and count_uppercase(part) > 1 and part.strip())]

        # Filter if there is only a number or a number followed by a dot
        filtered_parts = [part for part in filtered_parts if not bool(re.match(r'^\d+\.?$', part.strip()))]
    ...
    def concatenate_segments_v3(self, segment):
        ...
        if len(segment) > 0:
          ...
          for i, line in enumerate(lines):
              ...
              if re.match(r'^\s*[A-Z]', line) or re.match(r'^-\s*[A-Z]', line) or line == " - ":

                  if i > 0 and line != " - ":

                      # Check if the previous line ends with a word of more than 3 lowercase characters followed by a space
                      if re.match(r'.*\b[a-z]{1,3}\s$', lines[i - 1]) or lines[i - 1] == " - " :
